...
export default function App() {
  ...
  // Dữ liệu xã Thạnh Hưng từ Firestore (thay thế COMMUNE_DATA['X. Thạnh Hưng'])
  const currentZoneData = useMemo(() => {
    return communeData['X. Thạnh Hưng'] || EMPTY_COMMUNE_ENTRY;
  }, [communeData]);
  ...
  const [aiChatHistory, setAiChatHistory] = useState([
    {
      role: 'assistant',
      text: 'Xin chào Ban chỉ đạo Xã Thạnh Hưng! Tôi là Trợ lý AI Điều Hành DECC. Tôi đã đối soát hoàn chỉnh cả 2 Tầng dữ liệu: Tầng 1 (Chuẩn Phường/Xã 2.0 - DEI 76%) và Tầng 2 (Hệ sinh thái Kinh tế số). Sẵn sàng đồng hành cùng lãnh đạo để phân tích mâu thuẫn hệ thống và ra quyết định chính xác nhất.',
      timestamp: '07:41',
    },
  ]);
      ...
      contents: [
        {
          parts: [
            {
              text: 'Soạn một quyết định hành chính ngắn gọn để khắc phục điểm nghẽn đào tạo chuyển đổi số tại Xã Thạnh Hưng.',
            },
          ],
        },
      ],
                ...
                <div className="bg-[#0A2540] border border-cyan-500/15 rounded-3xl p-5 flex flex-col">
                  <div className="flex items-center gap-2 mb-5">
                    ...
                    <span className="text-sm font-bold text-cyan-400 uppercase tracking-wide">
                      B1: THÔNG TIN ĐƠN VỊ KINH DOANH - XÃ THẠNH HƯNG
                    </span>
                  </div>
      ...
      <footer className="bg-[#0A2540] border-t border-cyan-500/10 px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4 mt-auto text-xs text-slate-400">
        <div className="flex items-center gap-2">
          ...
          <span>
            ...
            mật IOC xã Thạnh Hưng.
          </span>
        </div>
        <div>
          <span>© 2026 Ban Chỉ Đạo Chuyển Đổi Số Xã Thạnh Hưng.</span>
        </div>
      </footer>
    </div>
  );
